...
const colorScale = d3
  .scaleQuantize()
  .range(["#cce5df", "#aee1d4", "#86d1c0", "#5cbea9", "#00a688"]);
...
function parseUnlockData(unlockRaw) {
  ...
  const dateColumns = unlockRaw.columns.filter(
    (col) => col !== "Unnamed: 0" && col !== "Total Usage"
  );
  ...
  dateColumns.forEach((rawDateStr) => {
    ...
    const totalUnlocks = d3.sum(unlockRaw, (d) =>
      parseFloat(d[rawDateStr] || 0)
    );
...
async function transformData(rawData, unlockMap) {
  const dateColumns = rawData.columns.filter(
    (col) =>
...
function renderTimeline(data, svg) {
  ...
  const contentHeight =
    data.length * TIMELINE_DAY_HEIGHT + TIMELINE_PADDING * 2;
  ...
  currentTimelineY = Math.max(
    minDragY,
    Math.min(
      maxDragY,
      currentTimelineY === undefined ? maxDragY : currentTimelineY
    )
  );
...
function showModal(event, d) {
  ...
  modal.select("#modal-total-usage").html(`
        ...
        <span style="font-size: 0.9em; color: #ccc;">Unlocks: ${
          d.unlockCount
        }</span>
    `);
  ...
  d.apps.slice(0, 5).forEach((app) => {
    appList.append("div").attr("class", "app-item").html(`
  ...
  // 4. [步驟四] 套用位置
  modal.style("left", `${finalX}px`).style("top", `${finalY}px`);
}
...
function renderLegend(scale) {
  ...
  scale.range().forEach((color) => {
    ...
    const text = `${formatTime(Math.round(extent[0]))} - ${formatTime(
      Math.round(extent[1])
    )}`;
...
async function main() {
  try {
    ...
    visContainer.text(
      "Data loading failed. Please check if app_usage.csv exists, or check browser console for errors."
    );
  }
}
